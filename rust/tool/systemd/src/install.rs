...
impl Installer {
    ...
    fn install_generation(&mut self, generation: &Generation) -> Result<()> {
        ...
        let kernel_dirname = bootspec
            ...
            .with_context(|| "Failed to extract kernel directory name")?;
        let kernel_version = kernel_dirname.rsplit('-').next().unwrap();

        // Install the kernel and record its path on the ESP.
        let kernel_target = self
            .install_ca(&bootspec.kernel, &format!("kernel-{}", kernel_version))
        ...
        let initrd_target = self
            .install_ca(&initrd_location, &format!("initrd-{}", kernel_version))
    ...
    fn register_installed_generation(&mut self, generation: &Generation) -> Result<()> {
        ...
        if !kernel_path.exists() && !initrd_path.exists() {
            None.context("Missing kernel or initrd.")?;
        }
        self.gc_roots
...
fn atomic_copy(from: &Path, to: &Path) -> Result<()> {
    let tmp = to.with_extension(".tmp");
    {
        ...
        let mut tmp_file =
            File::create(&tmp).with_context(|| "Failed to create the temporary file {tmp:?}")?;
